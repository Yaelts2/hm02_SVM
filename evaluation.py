...
def plot_decision_boundary(model, X, y, title="Decision Boundary"):
    """
    מצייר את גבול ההחלטה של מודל סיווג בדו-מימד.
    """
    # טווח הציור – קצת מעבר לגבולות הדאטה
    x_min, x_max = X[:, 0].min() - 0.5, X[:, 0].max() + 0.5
    y_min, y_max = X[:, 1].min() - 0.5, X[:, 1].max() + 0.5

    # רשת צפופה של נקודות לציור הגבול
    xx, yy = np.meshgrid(
        np.linspace(x_min, x_max, 300),
        np.linspace(y_min, y_max, 300)
    )

    # תחזית על כל הנקודות ברשת
    grid_points = np.c_[xx.ravel(), yy.ravel()]
    Z = model.predict(grid_points)
    Z = Z.reshape(xx.shape)
...
def plot_accuracy_bars(accuracies, labels, title="Model Comparison", ylabel="Accuracy"):
    """
    מצייר bar chart להשוואת ערכים (Accuracy / F1 וכו') בין מודלים.
    """
    plt.figure(figsize=(6, 4))
    ...
    bars = plt.bar(
        x,
        accuracies,
        color=["red", "blue", "green"][:len(labels)]  # צבעים פשוטים
    )

    # ערך מספרי מעל כל עמודה
